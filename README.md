...
言語・テーマ・API キー・プロンプトなどは `chrome.storage.local` に保存され、端末内にのみ保持されます。UI 文言と要約プロンプトの初期値も言語に合わせて切り替わります。ヘッダーのアイコンでライト／ダークを切り替えできます。

## 使い方

1. 要約したいWebページを開く
2. 拡張機能アイコンをクリックしてサイドパネルを開く
3. API キー・モデル・話者・要約プロンプト（任意）を設定する
